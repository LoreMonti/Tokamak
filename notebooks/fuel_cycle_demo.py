...
2. Evoluzione dell'inventario per TBR < 1, = 1, > 1.
...
def main() -> None:
    ...
    axes[0].set_xlabel("potenza di fusione [GW]")
    axes[0].set_ylabel("consumo di trizio [kg/giorno]")
    axes[0].set_title("Consumo di trizio")
    ...
    axes[1].set_xlabel("tempo [anni]")
    axes[1].set_ylabel("inventario di trizio [kg]")
    axes[1].set_title("Evoluzione dell'inventario")
    axes[1].legend()
    ...
    axes[2].set_ylabel("doubling time [anni]")
    axes[2].set_title("Tempo di raddoppio (avvio nuovi reattori)")
