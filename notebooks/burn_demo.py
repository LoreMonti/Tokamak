...
def main() -> None:
    ...
    axes[0].set_title("Self-consistent D-T burn")
    axes[0].grid(True, alpha=0.3)

    axes[1].plot(s.time, s.n_DT / 1e20, color="navy", lw=2, label="D-T fuel")
    axes[1].plot(s.time, s.n_He / 1e20, color="orange", lw=2, label="He ash")
    axes[1].axvline(t_off, color="gray", ls=":", lw=1)
    axes[1].set_ylabel(r"density [$10^{20}$ m$^{-3}$]")
    ...
    axes[2].plot(s.time, p_fus / 1e3, color="black", lw=1, alpha=0.6, label=r"$P_{fus}$ total")
    axes[2].axvline(t_off, color="gray", ls=":", lw=1, label="heating off")
    axes[2].set_ylabel(r"power [kW/m$^3$]")
